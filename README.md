...
* `scopeEnd: "scope"|"tree" = "scope"` - Specifies where the scope ends:
  ...
  - As from v0.2.0, this setting may be overwritten in individual files by using the `@scope` directive at the very top (if using sass/scss you need to put it just after any `@use` rules). Both `@scope tree;` and `@scope scope;` are valid.
